package com.ryuqq.authhub.adapter.out.persistence.architecture.adapter.command;
...
class CommandAdapterArchTest {
    ...
    class MethodRules {
        ...
        /**
         * 규칙 3-5: @Override 어노테이션 (권장)
         *
         * <p>Port 인터페이스 구현 메서드에 @Override를 권장하지만, 컴파일러가 이미 인터페이스 구현을 검증하므로 엄격하게 강제하지 않습니다.
         */
        @Test
        @DisplayName("[권장] persist() 메서드에 @Override 어노테이션이 권장됩니다")
        void commandAdapter_PersistMethodShouldHaveOverrideAnnotation() {
            // Port 인터페이스 구현은 컴파일 타임에 검증됨
            // @Override는 권장 사항이므로 별도 규칙 검증 생략
            // 실제 코드 리뷰에서 확인
        }
