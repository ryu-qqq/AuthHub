...
JWT 발급, 사용자/테넌트/조직/역할/권한 관리를 담당합니다.
...
│  │ 3. Permission 검사 (@PreAuthorize 어노테이션 기반)                          │  │
...
| **AuthHub**
| 인증(JWT 발급), JWKS 제공, 사용자/테넌트/조직/역할/권한 관리
...
----
┌─────────────────────────────────────────────────────────────────────────────────┐
│                     @PreAuthorize 기반 권한 검사                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │ 1. Gateway에서 JWT 검증 및 헤더 주입                                      │    │
│  │    - X-User-Id, X-Tenant-Id, X-Roles, X-Permissions                      │    │
│  │                                                                          │    │
│  │ 2. AuthHub Controller에서 @PreAuthorize로 권한 검사                       │    │
│  │    - @PreAuthorize("@access.hasPermission('user:read')")                 │    │
│  │    - @PreAuthorize("@access.hasRole('SUPER_ADMIN')")                     │    │
│  │    - @PreAuthorize("@access.superAdmin()")                               │    │
│  │                                                                          │    │
│  │ 3. 권한 없으면 403 Forbidden 반환                                         │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────────┘
----
...
| GET
| /api/v1/auth/permissions/users/{userId}
| SUPER_ADMIN
| 사용자 권한 조회

| POST
| /api/v1/auth/system/tenants/onboarding
| **X-Service-Token**
| 테넌트 온보딩 (입점 승인)
|===
...
include::v1/system/system.adoc[]

include::v1/internal/internal.adoc[]
