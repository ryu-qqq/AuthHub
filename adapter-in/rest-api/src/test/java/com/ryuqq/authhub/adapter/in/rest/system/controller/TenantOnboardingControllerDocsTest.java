...
/**
 * TenantOnboardingController REST API 문서화 테스트
 *
 ...
 * </ul>
 *
 * <p><strong>인증:</strong> X-Service-Token 헤더 (서버 간 통신용)
 *
...
@DisplayName("TenantOnboardingController REST Docs")
@Tag("docs")
class TenantOnboardingControllerDocsTest extends RestDocsTestSupport {

    @MockBean private TenantOnboardingUseCase tenantOnboardingUseCase;

    @MockBean private TenantOnboardingApiMapper mapper;
    ...
    void onboardTenant() throws Exception {
        // Given
        TenantOnboardingApiRequest request =
                new TenantOnboardingApiRequest(
                        "Acme Corporation", "admin@acme.com", "홍길동", "기본 조직");

        UUID tenantId = UUID.randomUUID();
        UUID organizationId = UUID.randomUUID();
        UUID userId = UUID.randomUUID();
        String temporaryPassword = "a1b2c3d4e5f6";

        TenantOnboardingCommand command =
                TenantOnboardingCommand.of("Acme Corporation", "admin@acme.com", "홍길동", "기본 조직");

        TenantOnboardingResponse useCaseResponse =
                TenantOnboardingResponse.of(tenantId, organizationId, userId, temporaryPassword);

        TenantOnboardingApiResponse apiResponse =
                TenantOnboardingApiResponse.of(tenantId, organizationId, userId, temporaryPassword);

        given(mapper.toCommand(any(TenantOnboardingApiRequest.class))).willReturn(command);
                ...
                .andDo(
                        document(
                                ...
                                requestFields(
                                        ...
                                        fieldWithPath("masterName")
                                                .description("마스터 관리자 이름 (1-50자)"),
                                        fieldWithPath("defaultOrgName")
                                ...
                                responseFields(
                                        ...
                                        fieldWithPath("data.temporaryPassword")
                                                .description("임시 비밀번호 (호출자가 이메일로 발송해야 함)"),
