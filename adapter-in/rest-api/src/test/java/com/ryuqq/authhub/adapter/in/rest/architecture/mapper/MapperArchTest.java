package com.ryuqq.authhub.adapter.in.rest.architecture.mapper;
...
class MapperArchTest {
    ...
    class DependencyProhibitionRules {
        ...
        void mapper_MustNotUseDomainObjects() {
            // Note: Domain VO(특히 Enum)는 Application Command에서 사용되므로,
            // Mapper에서 String → Domain Enum 변환은 허용합니다 (예: parseType, parseScope)
            // 실제로 금지해야 하는 것은 Domain Aggregate 직접 사용
            ArchRule rule =
                    noClasses()
                            .that()
                            .resideInAPackage("..mapper..")
                            ...
                            .allowEmptyShould(true)
                            .because(
                                    "Mapper는 Domain Aggregate를 직접 의존하면 안 됩니다"
                                            + " (VO/Enum, ErrorMapper는 예외)");
