# AUT-000: Domain Skeleton + Infrastructure 설정

**Epic**: IAM Platform (AuthHub)
**Feature**: 공통 기반 (Foundation)
**Phase**: Phase 0
**브랜치**: feature/AUT-000-foundation
**의존성**: 없음 (최초 Task)

---

## 📝 목적

모든 기능의 공통 기반 마련
- 핵심 Domain Aggregate 기본 구조 (최소 속성만)
- Infrastructure 설정 (Flyway, HikariCP, Redis)
- Multi-module 프로젝트 구조 세팅

---

## 🎯 요구사항

### 1. Domain Layer

#### 1.1 User Aggregate (최소 구조)
- [ ] **User Aggregate Root** 기본 구조
  - userId (UserId VO, UUID)
  - tenantId (Long FK)
  - organizationId (Long FK, Nullable)
  - userType (Enum: PUBLIC, INTERNAL)
  - status (UserStatus Enum: ACTIVE, INACTIVE, SUSPENDED, DELETED)
  - createdAt, updatedAt (LocalDateTime)

- [ ] **UserId VO** (UUID 기반)
- [ ] **UserType Enum** (PUBLIC, INTERNAL)
- [ ] **UserStatus Enum** (ACTIVE, INACTIVE, SUSPENDED, DELETED)

#### 1.2 Tenant Aggregate (최소 구조)
- [ ] **Tenant Aggregate Root** 기본 구조
  - tenantId (TenantId VO, Long)
  - name (TenantName VO)
  - status (TenantStatus Enum: ACTIVE, INACTIVE, DELETED)
  - createdAt, updatedAt

- [ ] **TenantId VO** (Long 기반)
- [ ] **TenantName VO** (String, 검증 로직)
- [ ] **TenantStatus Enum**

#### 1.3 Organization Aggregate (최소 구조)
- [ ] **Organization Aggregate Root** 기본 구조
  - organizationId (OrganizationId VO, Long)
  - tenantId (Long FK)
  - name (OrganizationName VO)
  - status (OrganizationStatus Enum: ACTIVE, INACTIVE, DELETED)
  - createdAt, updatedAt

- [ ] **OrganizationId VO** (Long 기반)
- [ ] **OrganizationName VO** (2-100자, 검증 로직)
- [ ] **OrganizationStatus Enum**

### 2. Application Layer

**이 단계에서는 UseCase 구현하지 않음** (기본 Port만 정의)

- [ ] **Port 인터페이스 패키지 구조**
  - `port.in.command` (Command UseCase 인터페이스)
  - `port.in.query` (Query UseCase 인터페이스)
  - `port.out.command` (Command Port 인터페이스)
  - `port.out.query` (Query Port 인터페이스)

### 3. Persistence Layer

#### 3.1 JPA Entity (기본 구조만)
- [ ] **UserEntity**
  - id, tenant_id, organization_id, user_type, status
  - created_at, updated_at
  - BaseAuditEntity 상속

- [ ] **TenantEntity**
  - id, name, status, created_at, updated_at

- [ ] **OrganizationEntity**
  - id, tenant_id, name, status, created_at, updated_at

- [ ] **BaseAuditEntity** (공통 감사 필드)
  - createdAt, updatedAt
  - @EntityListeners(AuditingEntityListener.class)

#### 3.2 Flyway 마이그레이션
- [ ] **V1__init_schema.sql**
  - tenants 테이블 생성
  - organizations 테이블 생성
  - users 테이블 생성 (기본 컬럼만)
  - Index 및 Unique Constraint

- [ ] **V2__insert_default_data.sql**
  - "Connectly Public Tenant" 삽입
  - 시스템 기본 Organization (선택)

#### 3.3 HikariCP 설정
- [ ] **application.yml** (datasource 설정)
  - maximumPoolSize: 10
  - minimumIdle: 5
  - connectionTimeout: 30000
  - idleTimeout: 600000
  - maxLifetime: 1800000

#### 3.4 Redis 설정
- [ ] **RedisConfig.java**
  - RedisConnectionFactory Bean
  - RedisTemplate Bean
  - StringRedisTemplate Bean

- [ ] **application.yml** (redis 설정)
  - host: localhost
  - port: 6379
  - timeout: 3000ms

### 4. REST API Layer

**이 단계에서는 Controller 구현하지 않음** (공통 DTO만 정의)

- [ ] **ApiResponse<T>** (공통 응답 DTO)
  - success (Boolean)
  - data (T)
  - timestamp (LocalDateTime)

- [ ] **PageApiResponse<T>** (페이징 응답 DTO)
  - content (List<T>)
  - totalElements (Long)
  - totalPages (int)
  - currentPage (int)

- [ ] **ErrorInfo** (공통 에러 DTO)
  - errorCode (String)
  - message (String)
  - details (Map<String, Object>)

- [ ] **GlobalExceptionHandler** (기본 구조만)

### 5. Integration Test

- [ ] **TestConfiguration**
  - ClockConfig (고정된 시각)
  - EmbeddedRedis 설정 (테스트용)

- [ ] **Flyway 마이그레이션 테스트**
  - V1, V2 순차 실행 검증
  - 스키마 생성 확인
  - 기본 데이터 삽입 확인

---

## ⚠️ 제약사항

### Zero-Tolerance 규칙
- [ ] **Lombok 금지** - Pure Java 또는 Record 사용
- [ ] **Long FK 전략** - JPA 관계 어노테이션 금지
  - `private Long tenantId` ✅
  - `@ManyToOne private Tenant tenant` ❌

### 최소 구현 원칙
- [ ] **YAGNI 준수** - 이 단계에서는 최소 속성만 구현
- [ ] **확장 가능 구조** - 향후 기능에서 속성 추가 가능하도록 설계

### 테스트 규칙
- [ ] **ArchUnit 테스트** 필수
  - Lombok 사용 금지 검증
  - Long FK 전략 검증
  - 레이어 의존성 검증
- [ ] **Flyway 마이그레이션 테스트**
- [ ] **테스트 커버리지** > 80%

---

## ✅ 완료 조건

- [ ] Domain Layer 기본 구조 완료 (User, Tenant, Organization)
- [ ] Persistence Layer 기본 구조 완료 (Entity, Flyway)
- [ ] Infrastructure 설정 완료 (HikariCP, Redis)
- [ ] 공통 DTO 구조 완료
- [ ] 모든 테스트 통과 (Unit + ArchUnit + Flyway)
- [ ] Zero-Tolerance 규칙 준수
- [ ] 코드 리뷰 승인
- [ ] PR 머지 완료

---

## 🔗 관련 문서

- **PRD**: docs/prd/iam-platform.md
- **Plan**: docs/prd/plans/AUT-000-foundation-plan.md (create-plan 후 생성)
- **Jira**: https://ryuqqq.atlassian.net/browse/AUT-53

---

## 📌 참고 사항

이 Task는 **모든 기능의 공통 기반**입니다.
- 최소한의 Domain 구조만 제공
- 각 기능(AUT-001~)에서 필요한 속성/메서드 추가
- Infrastructure는 한 번만 설정
