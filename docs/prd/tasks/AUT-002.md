# AUT-002: Application Layer 구현

**Epic**: B2B AuthHub - 멀티 테넌트 인증/인가 시스템
**Issue Key**: AUT-002
...
B2B AuthHub의 비즈니스 유스케이스를 구현하는 Application Layer를 TDD로 개발합니다.

**핵심 원칙**:
- UseCase 단위 트랜잭션 경계 명확화
- CQRS 패턴 엄격 준수 (Command/Query 분리)
- Assembler를 통한 Domain ↔ DTO 변환
...
public class LoginUseCaseImpl implements LoginUseCase {
    ...
    @Override
    @Transactional(readOnly = true) // 외부 API 호출 없음
...
public record LoginCommand(
    ...
    public LoginCommand {
        requireNonNull(username, "username은 필수입니다");
        requireNonNull(password, "password는 필수입니다");
    }
}
```
...
public record LoginResponse(
    ...
    public static LoginResponse of(String accessToken, String refreshToken) {
        return new LoginResponse(accessToken, refreshToken, 900L); // 15분
    }
}
```

**다른 Auth UseCases**:
- `RefreshTokenUseCase`: Refresh Token으로 Access Token 재발급 (RTR 적용)
- `LogoutUseCase`: Refresh Token 무효화
- `ValidateTokenUseCase`: Gateway용 JWT 검증
- `RevokeAllTokensUseCase`: 특정 사용자의 모든 Refresh Token 무효화

#### 1.2 User UseCases (7개)
...
public class CreateUserUseCaseImpl implements CreateUserUseCase {
    ...
    @Override
    @Transactional // Command UseCase는 기본 트랜잭션
...
**다른 User UseCases**:
- `UpdateUserUseCase`: 사용자 정보 수정
- `DeleteUserUseCase`: 사용자 삭제 (Soft Delete)
- `SuspendUserUseCase`: 사용자 정지
- `ChangePasswordUseCase`: 비밀번호 변경
- `GetUserUseCase` (Query): 사용자 조회
- `SearchUsersUseCase` (Query): 사용자 목록 조회 (페이징)

#### 1.3 Organization UseCases (5개)
...
**다른 Organization UseCases**:
- `CreateOrganizationUseCase`: 조직 생성
- `UpdateOrganizationUseCase`: 조직 정보 수정
- `DeactivateOrganizationUseCase`: 조직 비활성화
- `GetOrganizationUseCase` (Query): 조직 조회

#### 1.4 Role & Permission UseCases (4개)
...
**다른 Role & Permission UseCases**:
- `RevokeRoleFromUserUseCase`: 역할 제거
- `GetUserPermissionsUseCase` (Query): 사용자 권한 조회 (캐시 활용)
- `GetRolePermissionsUseCase` (Query): 역할별 권한 조회

#### 1.5 Tenant UseCases (3개)
...
**다른 Tenant UseCases**:
- `UpdateTenantUseCase`: 테넌트 정보 수정
- `SuspendTenantUseCase`: 테넌트 정지

---

### 2. Ports 정의 (Command/Query 분리)
...
public class UserAssembler {
    ...
    public User toDomain(CreateUserCommand command) {
        return User.create(
            ...
            Username.of(command.username()),
            Email.of(command.email()),
            Password.of(command.password()), // 이미 인코딩된 비밀번호
            PhoneNumber.of(command.phoneNumber())
        );
    }
...
**Assembler 규칙**:
- Domain → Response: 모든 필드 매핑
- Command → Domain: VO 변환 및 검증
- VO의 value() 메서드 호출하여 원시 타입 추출

---

## ✅ Zero-Tolerance Rules Checklist

### Application Layer 필수 규칙
...
- [ ] **Command는 Transactional**: 기본적으로 `@Transactional` 적용
- [ ] **Query는 readOnly**: 조회는 `@Transactional(readOnly = true)`
- [ ] **UseCase 단일 책임**: 하나의 UseCase는 하나의 비즈니스 로직만
- [ ] **Domain 객체 직접 반환 금지**: 항상 Response DTO로 변환

### 금지된 패턴 예시

❌ **잘못된 예 (Transaction 내 외부 API 호출)**:
```java
@Transactional
...
public void execute(CreateUserCommand command) {
    ...
    // ✅ Event 발행 (트랜잭션 완료 후 Listener가 처리)
...
│   │   └── ...
│   └── organization/
│       └── DeleteOrganizationUseCaseTest.java
└── assembler/
...
class CreateUserUseCaseTest {
    ...
    @Test
    @DisplayName("중복된 username으로 생성 실패")
    ...
    @Test
    @DisplayName("Organization이 비활성화 상태일 때 생성 실패")
...
class UserAssemblerTest {
    ...
    @Test
    @DisplayName("User를 UserResponse로 변환 성공")
...
### 1. 구현 완료 조건
- [ ] 24개 UseCase 모두 구현 완료 (18 Command + 6 Query)
- [ ] Port 인터페이스 모두 정의 완료
- [ ] Assembler 모두 구현 완료
...
- [ ] Transaction 경계 적절성 확인
- [ ] CQRS 분리 확인
- [ ] Port 인터페이스 명명 규칙 확인
- [ ] Assembler 사용 확인
- [ ] Domain 객체 직접 노출 여부 확인
...
## 🔄 Kent Beck TDD Commands

```bash
# Application Layer TDD 사이클 실행
/kb/application/go          # Plan 파일 기반 다음 테스트 실행
/kb/application/red         # Red: test: 커밋
/kb/application/green       # Green: feat: 커밋
/kb/application/refactor    # Refactor: struct: 커밋
```
