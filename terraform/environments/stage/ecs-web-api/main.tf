...
# Internal access via: authhub.connectly.local:8080
# ========================================
...
resource "aws_kms_key" "logs" {
  description             = "KMS key for AuthHub web-api CloudWatch logs encryption (${var.environment})"
...
module "web_api_logs" {
  ...
  retention_in_days = 14  # Stage: 짧은 보관 기간
...
module "ecs_security_group" {
  ...
  description = "Security group for web-api ECS tasks (${var.environment})"
  vpc_id      = local.vpc_id
      ...
      policy = jsonencode({
        ...
        Statement = [
          ...
          },
          {
            ...
            Resource = [
              "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/ecs/authhub/otel:*",
              "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/ecs/authhub/otel-collector:*"
            ]
          },
          {
