...
/**
 * ServiceSearchCriteria - 서비스 검색 조건 Criteria
 *
 * <p>서비스 목록 조회 시 사용하는 검색 조건을 정의합니다.
 *
...
public record ServiceSearchCriteria(
     ...
     */
    public static ServiceSearchCriteria of(
            String searchWord,
            ServiceSearchField searchField,
            List<ServiceStatus> statuses,
            DateRange dateRange,
        ...
        QueryContext<ServiceSortKey> context = QueryContext.of(sortKey, sortDirection, pageRequest);
        return new ServiceSearchCriteria(searchWord, searchField, statuses, dateRange, context);
    }
     ...
     * @return ServiceSearchCriteria
     */
    public static ServiceSearchCriteria ofSimple(
            String searchWord,
            List<ServiceStatus> statuses,
            DateRange dateRange,
            int pageNumber,
            int pageSize) {
        QueryContext<ServiceSortKey> context =
                QueryContext.of(
                        ServiceSortKey.CREATED_AT,
                        SortDirection.DESC,
                        PageRequest.of(pageNumber, pageSize));
        return new ServiceSearchCriteria(
                searchWord, ServiceSearchField.defaultField(), statuses, dateRange, context);
    }
