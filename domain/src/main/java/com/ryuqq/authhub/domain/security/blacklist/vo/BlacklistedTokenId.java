...
/**
 * BlacklistedToken의 고유 식별자를 나타내는 Value Object.
 * UUID 기반으로 전역적으로 유일한 블랙리스트 토큰 식별자를 제공합니다.
 *
...
public record BlacklistedTokenId(UUID value) {
    ...
    public BlacklistedTokenId {
        if (value == null) {
            throw new IllegalArgumentException("BlacklistedTokenId value cannot be null");
        }
    }

    /**
     * 새로운 UUID를 생성하여 BlacklistedTokenId를 만듭니다.
     *
     * @return 새로 생성된 BlacklistedTokenId 인스턴스
     * @author AuthHub Team
     * @since 1.0.0
     */
    public static BlacklistedTokenId newId() {
        return new BlacklistedTokenId(UUID.randomUUID());
    }

    /**
     * 기존 UUID 문자열로부터 BlacklistedTokenId를 생성합니다.
     *
     * @param uuidString UUID 문자열 (null 불가)
     * @return BlacklistedTokenId 인스턴스
     * @throws IllegalArgumentException uuidString이 null이거나 유효하지 않은 UUID 형식인 경우
     * @author AuthHub Team
     * @since 1.0.0
     */
    public static BlacklistedTokenId fromString(final String uuidString) {
        if (uuidString == null || uuidString.trim().isEmpty()) {
            throw new IllegalArgumentException("UUID string cannot be null or empty");
        }
        try {
            return new BlacklistedTokenId(UUID.fromString(uuidString));
    ...
    /**
     * 기존 UUID 객체로부터 BlacklistedTokenId를 생성합니다.
     *
     ...
     * @since 1.0.0
     */
    public static BlacklistedTokenId from(final UUID uuid) {
        return new BlacklistedTokenId(uuid);
    }

    /**
     * BlacklistedTokenId를 문자열로 변환합니다.
     *
     * @return UUID 문자열 표현
     * @author AuthHub Team
     * @since 1.0.0
