...
/**
 * AuditLog의 고유 식별자를 나타내는 Value Object.
 * UUID 기반으로 전역적으로 유일한 감사 로그 식별자를 제공합니다.
 *
...
public record AuditLogId(UUID value) {
    ...
    public AuditLogId {
        if (value == null) {
            throw new IllegalArgumentException("AuditLogId value cannot be null");
        }
    }

    /**
     * 새로운 UUID를 생성하여 AuditLogId를 만듭니다.
     *
     * @return 새로 생성된 AuditLogId 인스턴스
     * @author AuthHub Team
     * @since 1.0.0
     */
    ...
    /**
     * 기존 UUID 문자열로부터 AuditLogId를 생성합니다.
     *
     ...
     * @since 1.0.0
     */
    public static AuditLogId fromString(final String uuidString) {
        if (uuidString == null || uuidString.trim().isEmpty()) {
            throw new IllegalArgumentException("UUID string cannot be null or empty");
        }
        try {
            return new AuditLogId(UUID.fromString(uuidString));
    ...
    /**
     * 기존 UUID 객체로부터 AuditLogId를 생성합니다.
     *
     ...
     * @since 1.0.0
     */
    public static AuditLogId from(final UUID uuid) {
        return new AuditLogId(uuid);
    }

    /**
     * AuditLogId를 문자열로 변환합니다.
     *
     * @return UUID 문자열 표현
     * @author AuthHub Team
     * @since 1.0.0
