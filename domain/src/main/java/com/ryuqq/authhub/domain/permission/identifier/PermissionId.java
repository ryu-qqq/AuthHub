...
public final class PermissionId {
    ...
    /**
     * 새로운 PermissionId 생성 (Application Layer에서 UUID 생성 후 전달)
     *
     * @param uuid Application Layer에서 생성된 UUIDv7
     * @return 새로운 PermissionId 인스턴스
     */
    public static PermissionId forNew(UUID uuid) {
        return new PermissionId(uuid);
    }

    /**
     * 기존 UUID로 PermissionId 생성 (DB 조회 시 사용)
     *
     * @param value UUID 값
    ...
    public static PermissionId of(String value) {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException("PermissionId 문자열은 null이거나 빈 값일 수 없습니다");
        }
        try {
