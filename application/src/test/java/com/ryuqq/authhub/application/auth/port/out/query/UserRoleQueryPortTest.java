...
@Tag("unit")
@Tag("port")
@DisplayName("UserRoleQueryPort 인터페이스 설계 테스트")
class UserRoleQueryPortTest {
    ...
    class InterfaceStructureTest {
        ...
        @Test
        @DisplayName("[필수] UserRoleQueryPort는 인터페이스여야 한다")
        void shouldBeInterface() {
            // When & Then
            assertThat(UserRoleQueryPort.class.isInterface())
                    .as("UserRoleQueryPort는 interface로 선언되어야 합니다")
                    .isTrue();
        }

        @Test
        @DisplayName("[필수] UserRoleQueryPort는 public이어야 한다")
        void shouldBePublic() {
            // When & Then
            assertThat(Modifier.isPublic(UserRoleQueryPort.class.getModifiers()))
                    .as("UserRoleQueryPort는 public으로 선언되어야 합니다")
                    .isTrue();
        }
    }
    ...
    class FindByUserIdMethodTest {
        ...
        void shouldHaveFindByUserIdMethod() throws NoSuchMethodException {
            // When
            Method findMethod = UserRoleQueryPort.class.getMethod("findByUserId", UserId.class);
        ...
        @Test
        @DisplayName("[필수] findByUserId() 메서드는 Set을 반환해야 한다")
        void findByUserIdShouldReturnSet() throws NoSuchMethodException {
            // When
            Method findMethod = UserRoleQueryPort.class.getMethod("findByUserId", UserId.class);
            ...
            assertThat(findMethod.getReturnType())
                    .as("findByUserId() 메서드는 Set을 반환해야 합니다")
                    .isEqualTo(Set.class);

            // Generic 타입 검증 (Set<String>)
            if (returnType instanceof ParameterizedType parameterizedType) {
                Type[] typeArgs = parameterizedType.getActualTypeArguments();
                assertThat(typeArgs).hasSize(1);
                assertThat(typeArgs[0])
                        .as("Set의 Generic 타입은 String이어야 합니다")
    ...
    class ExistsByUserIdMethodTest {
        ...
        void shouldHaveExistsByUserIdMethod() throws NoSuchMethodException {
            // When
            Method existsMethod = UserRoleQueryPort.class.getMethod("existsByUserId", UserId.class);
    ...
    class ProhibitedMethodsTest {
        ...
        void shouldNotHaveSaveOrPersistMethod() {
            ...
            boolean hasProhibitedMethod = Arrays.stream(UserRoleQueryPort.class.getMethods())
                    .map(Method::getName)
        ...
        void shouldNotHaveFindAllMethod() {
            // When
            boolean hasFindAllMethod = Arrays.stream(UserRoleQueryPort.class.getMethods())
