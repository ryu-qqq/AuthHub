...
/**
 * UpdateOrganizationNameService - 조직 이름 변경 Service
 *
 * <p>UpdateOrganizationNameUseCase를 구현합니다.
 *
 ...
 */
@Service
public class UpdateOrganizationNameService implements UpdateOrganizationNameUseCase {
    ...
    public UpdateOrganizationNameService(
    ...
    @Override
    public void execute(UpdateOrganizationNameCommand command) {
        ...
        // 2. Validator: 기존 엔티티 조회 (없으면 예외)
        Organization organization = validator.findExistingOrThrow(context.id());

        // 3. Validator: 중복 이름 검증 (자신 제외)
        validator.validateNameNotDuplicatedExcluding(context.updateData(), organization);

        // 4. Domain: 이름 변경 적용
        organization.changeName(context.updateData(), context.changedAt());
