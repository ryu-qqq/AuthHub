package com.ryuqq.authhub.application.service.port.out.command;

import com.ryuqq.authhub.domain.service.aggregate.Service;

/**
 * ServiceCommandPort - Service Aggregate 영속화 포트 (Command)
 *
 ...
 * @since 1.0.0
 */
public interface ServiceCommandPort {

    /**
     * Service Aggregate 영속화 (생성/수정 통합)
     *
     * <p>Domain의 상태에 따라 적절한 영속화 작업을 수행합니다.
     *
     * <ul>
     *   <li>isNew() == true → INSERT (Auto Increment)
     *   <li>isNew() == false → UPDATE
     * </ul>
     *
     * @param service Service Aggregate
     * @return 영속화된 Service ID (Long)
     */
    Long persist(Service service);
}
