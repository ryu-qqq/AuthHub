...
public class SearchTenantsService implements SearchTenantsUseCase {
    ...
    public SearchTenantsService(TenantReadManager readManager, TenantAssembler assembler) {
    ...
    public PageResponse<TenantResponse> execute(SearchTenantsQuery query) {
        ...
        // 2. ReadManager: 조회
        List<Tenant> tenants =
                readManager.findAllByCriteria(query.name(), query.status(), offset, size);
        long totalElements = readManager.countAll(query.name(), query.status());
